...
Seeed Wiki: https://wiki.seeedstudio.com/ReSpeaker_6-Mic_Circular_Array_kit_for_Raspberry_Pi/
...
def verify(wav_path, min_energy=50, min_channels_with_signal=5):
    ...
    # Per-channel max absolute value (channels 0..5 are the 6 mics, Wiki)
    ch_max = [0] * 8
    ...
    if mic_ok < min_channels_with_signal:
        if mic_ok >= 1 and min_channels_with_signal >= 5:
            print(f"VERIFY_PARTIAL: {mic_ok}/6 mic channels; levels: {ch_max[:6]}")
            return True
