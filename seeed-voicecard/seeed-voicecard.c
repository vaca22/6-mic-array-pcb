...
/* Kernel 6.12+ removed rtd->id from snd_soc_pcm_runtime; use dai_link index */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6,12,0)
static inline int seeed_rtd_to_id(struct snd_soc_pcm_runtime *rtd)
{
	int i;
	for (i = 0; i < rtd->card->num_links; i++)
		if (rtd->card->dai_link + i == rtd->dai_link)
			return i;
	return 0;
}
#define seeed_rtd_id(rtd) seeed_rtd_to_id(rtd)
#else
#define seeed_rtd_id(rtd) ((rtd)->id)
#endif
...
static int seeed_voice_card_startup(struct snd_pcm_substream *substream)
{
	...
	struct seeed_dai_props *dai_props =
		seeed_priv_to_props(priv, seeed_rtd_id(rtd));
	int ret;
