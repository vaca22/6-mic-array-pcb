...
if [[ $? -eq 0 ]]; then
  apt update -y
  # Raspbian kernel packages (optional on Debian Trixie / RPi OS 2025)
  apt-get -y install raspberrypi-kernel-headers raspberrypi-kernel 2>/dev/null || true
  # Debian Trixie / RPi: use linux-headers-rpi-v8 or linux-headers-$(uname -r)
  if ! dpkg -L raspberrypi-kernel-headers &>/dev/null; then
    apt-get -y install linux-headers-rpi-v8 2>/dev/null || apt-get -y install linux-headers-$(uname -r) 2>/dev/null || true
  fi
  apt-get -y install gcc-aarch64-linux-gnu 2>/dev/null || true
  # Ubuntu kernel packages
  apt-get -y install linux-raspi linux-headers-raspi linux-image-raspi 2>/dev/null || true
  apt-get -y install dkms git i2c-tools libasound2-plugins
  # rpi-update checker (allow continuing if headers from uname -r are present)
  VER_RUN=$(get_kernel_version)
  if [ -z "$VER_RUN" ]; then VER_RUN=$(uname -r); fi
